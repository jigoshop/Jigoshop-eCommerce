...
class PhpFastCache implements ProductServiceInterface
{
    ...
    public function find($id)
    {
        if (!isset($this->objects[$id])) {
            $cachedProduct = $this->instanceCache->getItem('product_'.$id);
            if($cachedProduct->get() instanceof Product) {
                $this->objects[$id] = $cachedProduct->get();
            } else {
                $this->objects[$id] = $this->service->findForPost($id);
                $cachedProduct->set($this->objects[$id]);
