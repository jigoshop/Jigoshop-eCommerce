...
class Optimization
{
	...
	public function localizeScript($handle, $object, array $values)
	{
		...
		if (in_array($handle, $this->_scripts[$current_page]) || in_array($handle, $this->_scripts[self::JIGOSHOP_ALL])) {
	...
	private function checkStyleDependencies(array &$dependencies)
	{
		...
		foreach ($diff as $key => $dependency) {
			...
			if (!in_array($dependency, $this->_styles[self::JIGOSHOP_ALL]) && !in_array($dependency, $this->_styles[$current_page])) {
				return false;
			}
		}
