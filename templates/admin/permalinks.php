...
<table class="form-table">
	<tbody>
	<tr>
		<th><label><input name="product_permalink" type="radio" value="<?= $structures[0]; ?>" class="jigoshop-structure" <?= Forms::checked($structures[0], $permalink); ?> /> <?php _e('Default', 'jigoshop'); ?></label></th>
		<td><code><?= $homeUrl; ?>/?product=sample-product</code></td>
	</tr>
	<tr>
		<th><label><input name="product_permalink" type="radio" value="<?= $structures[1]; ?>" class="jigoshop-structure" <?= Forms::checked($structures[1], $permalink); ?> /> <?php _e('Product', 'jigoshop'); ?></label></th>
		<td><code><?= $homeUrl; ?>/<?= $productBase; ?>/sample-product/</code></td>
	</tr>
	<?php if ($shopPageId) : ?>
		<tr>
			<th><label><input name="product_permalink" type="radio" value="<?= $structures[2]; ?>" class="jigoshop-structure" <?= Forms::checked($structures[2], $permalink); ?> /> <?php _e('Shop base', 'jigoshop'); ?></label></th>
			<td><code><?= $homeUrl; ?>/<?= $base; ?>/sample-product/</code></td>
		</tr>
		<tr>
			<th><label><input name="product_permalink" type="radio" value="<?= $structures[3]; ?>" class="jigoshop-structure" <?= Forms::checked($structures[3], $permalink); ?> /> <?php _e('Shop base with category', 'jigoshop'); ?></label></th>
			<td><code><?= $homeUrl; ?>/<?= $base; ?>/product-category/sample-product/</code></td>
		</tr>
	<?php endif; ?>
	<tr>
		<th><label><input name="product_permalink" id="jigoshop_custom_selection" type="radio" value="custom" <?= Forms::checked(in_array($permalink, $structures), false); ?> /> <?php _e('Custom Base', 'jigoshop'); ?></label></th>
		<td>
