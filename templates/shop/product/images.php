...
<div class="images">
	<?php if (Product::isOnSale($product)): ?>
		<span class="on-sale"><?php echo apply_filters('jigoshop\template\product\sale_text', __('Sale!', 'jigoshop')) ?></span>
	<?php endif; ?>
	...
	<div class="thumbnails">
		<?php foreach($thumbnails as $id => $thumbnail): ?>
			<a href="<?php echo $thumbnail['url']; ?>" data-lightbox="product-gallery" data-title="<?php echo $thumbnail['title']; ?>" title="<?php echo $thumbnail['title']; ?>" class="zoom"><?php echo apply_filters('jigoshop\template\product\thumbnail', $thumbnail['image'], $thumbnail, $product); ?></a>
		<?php endforeach; ?>
	</div>
